...
image_features = pd.read_csv('./images_features_with_dnn.csv', header=None)

df_paths = pd.read_csv('../../Color Samples Dataset/dataset_register.csv')
df = pd.concat([image_features, df_paths], axis = 1)
print(df)
df = df.sample(frac = 1)
...
df_paths.to_csv('./dataset_register_with_classes_dnn.csv')

joblib.dump(kmeans, 'kmeans_class_generator_dnn.joblib')
print('Success...')
